...
import { getConfig, setConfig, updateUserEmby, getActiveMembership } from './db.js';

// 获取 Emby 配置
async function fetchConfig(db) {
  const baseR = await getConfig(db, 'emby_base_url');
  const keyR = await getConfig(db, 'emby_api_key');
  return {
...
export async function handleCheckConnection(request, env) {
  ...
  if (request.method === 'POST') {
    ...
    config = { baseUrl: body.baseUrl, apiKey: body.apiKey };
  } else {
...
export async function handleCreateEmbyAccount(request, env) {
  ...
  const body = await parseBody(request);
  const embyUsername = body.embyUsername;
  const embyPassword = body.embyPassword;
  if (!embyUsername || !embyPassword || embyUsername.length < 2 || embyPassword.length < 4) {
    return json({ error: 'invalid_input', message: '用户名至少2位，密码至少4位' }, 400);
  }
  const userId = request.session.userId;
  ...
  try {
    ...
    // 3. 启用用户
    await fetch(base + '/emby/Users/' + eid + '/Policy', {
      method: 'POST', headers: hdrs,
    ...
    return json({ ok: true, message: 'Emby 账号创建成功', data: { username: embyUsername, userId: eid } });
  } catch (e) {
